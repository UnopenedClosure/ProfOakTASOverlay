--[[
Note: the numberings are based on the Pokemon's number within the game's code, not their Pokedex number

Pending routing questions:
-are we taking Lileep or Cradily in PB4?
--]]

sectionData = {
	[1] = {
		sectionName = "Pre-Badge 1",
		sectionPokes = {--37 Pokes
			283, 284, 285,--Mudkip line
			286, 287, --Poochyena line
			288, 289, --Zigzagoon line
			290, 291, 292, 293, 294, --Wurmple lines
			295, 296, --Lotad line
			304, 305, --Taillow line
			309, 310, --Wingull line
			392, 393, 394, --Ralts line
			311, 312, --Surskit line
			306, 307, --Shroomish line
			364, 365, 366, --Slakoth line
			301, 302, 303, --Nincada line
			370, 371, 372, --Whismur line
			335, 336, --Makuhita line
			315 --Skitty
		}
	},
	[2] = {
		sectionName = "Pre-Badge 2",
		sectionPokes = {--30 Pokes
			41, 42, 169, --Zubat line
			43, 44, --Oddish line
			63, 64, --Abra line
			72, 73, --Tentacool line
			74, 75, --Geodude line
			118, 119, --Goldeen line
			129, 130, --Magikarp line
			183, 184, --Marill line
			322, --Sableye
			382, 383, 384, --Aron line
			337, 338, --Electrike line
			353, 354, --Plusle & Minun
			386, 387, --Volbeat & Illumise
			363, --Roselia
			367, 368 --Gulpin line
		}
	},
	[3] = {
		sectionName = "Pre-Badge 3",
		sectionPokes = {--25 Pokes
			27, 28, --Sandshrew line
			66, 67, --Machop line
			88, 89, --Grimer line
			109, 110, --Koffing line
			360, 202, --Wobbuffet line
			218, 219, --Slugma line
			227, --Skarmory
			320, --Nosepass
			316, --Delcatty
			339, 340, --Numel line
			321, --Torkoal
			351, 352, --Spoink line
			308, --Spinda
			358, 359, --Swablu line
			379, --Seviper
			348 --Lunatone
		}
	},
	[4] = {
		sectionName = "Pre-Badge 4",
		sectionPokes = {--9 Pokes
			332, 333, 334, --Trapinch line
			344, 345, --Cacnea line
			318, 319, --Baltoy line
			388, 389 --Lileep line 390, 391 --Anorith line
		}
	},
	[5] = {
		sectionName = "Pre-Badge 6",
		sectionPokes = {--59 Pokes
			172, 25, 26, --Pikachu line
			37, 38, --Vulpix line
			174, 39, 40, --Jigglypuff line
			45, 182, --Vileplume & Bellossom
			54, 55, --Psyduck line
			81, 82, --Magnemite line
			84, 85, --Doduo line
			100, 101, --Voltorb line
			111, 112, --Rhyhorn line
			116, 117, --Horsea line
			120, --Staryu
			127, --Pinsir
			177, 178, --Natu line
			203, --Girafarig
			214, --Heracross
			222, --Corsola line
			231, 232, --Phanpy line
			350, --Azurill
			356, 357, --Meditite line
			330, 331, --Carvanha line
			313, 314, --Wailmer line
			323, 324, --Barboach line
			326, 327, --Corphish line
			328, 329, --Feebas line
			385, --Castform
			317, --Kecleon
			377, 378, --Shuppet line
			361, 362, --Duskull line
			369, --Tropius
			411, --Chimecho
			376, --Absol
			346, 347, --Snorunt line
			341, 342, 343, --Spheal line
			325 --Luvdisc
		}
	},
	[6] = {
		sectionName = "Pre-Badge 7",
		sectionPokes = {--9 Pokes
			121, --Starmie
			170, 171, --Chinchou line
			297, --Ludicolo
			373, --Clamperl
			381, --Relicanth
			401, 402, --Regirock & Regice
			404 --Kyogre
		}
	},
	[7] = {
		sectionName = "Pre-Badge 8",
		sectionPokes = {--3 Pokes
			395, 396, 397 --Bagon line
		}
	},
	[8] = {
		sectionName = "Pre-Elite 4",
		sectionPokes = {--1 Poke
			403 --Registeel
		}
	},
	[9] = {
		sectionName = "Postgame",
		sectionPokes = {--5 Pokes
			398, 399, 400, --Beldum line
			407, --Latias
			406 --Rayquaza
		}
	}
}

itemXOffset = {
	[2]=3,
	[22]=4,
	[23]=5,
	[24]=7,
	[64]=5,
	[68]=2,
	[69]=5,
	[110]=5,
	[139]=2,
	[19]=4,
	[49]=6,
	[75]=2,
	[93]=1,
	[94]=2,
	[182]=1,
	[183]=4,
	[187]=1,
	[220]=2,
}

itemYOffset = {
	[2]=2,
	[22]=0,
	[23]=1,
	[24]=3,
	[64]=0,
	[68]=0,
	[69]=0,
	[110]=4,
	[139]=2,
	[19]=0,
	[49]=5,
	[75]=1,
	[93]=0,
	[94]=1,
	[182]=1,
	[183]=1,
	[187]=0,
	[220]=1,
}
